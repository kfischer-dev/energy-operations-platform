...
@pytest.mark.basic
def test_calculate_kpis_for_all_measurements_aggregates_multiple_assets():
    """Aggregate KPIs across multiple fully covered assets."""
    ...
    assert result["min_measured_power_kw"] == pytest.approx(100.0)
    assert result["max_measured_power_kw"] == pytest.approx(200.0)

    assert result["avg_active_power_kw"] == pytest.approx(300.0)
    assert result["total_energy_kwh"] == pytest.approx(150.0)

    assert result["coverage_ratio"] == pytest.approx(1.0)


@pytest.mark.basic
def test_calculate_kpis_for_all_measurements_averages_asset_coverage():
    """Average coverage across fully and partially covered assets."""

    start_time = datetime.fromisoformat("2026-06-22T08:00:00+02:00")
    end_time = datetime.fromisoformat("2026-06-22T08:30:00+02:00")

    measurements = [
        # Asset 1: full 30-minute coverage
        {
            "asset_id": 1,
            "measurement_time": start_time,
            "active_power_kw": 100.0,
