...
def test_offline_asset_returns_zero_power(engine_payload, operating_status):
    # Create the selected asset in an inactive operating state.
...
def test_simulation_generates_expected_number_of_measurements(engine_payload):
    # The default time grid currently contains nine timestamps.
    simulated_power_measurement_list = simulate_asset_power_grid(
        engine_payload["config"],
        engine_payload["asset"],
    )

    assert len(simulated_power_measurement_list) == 9

@pytest.mark.simulation
@pytest.mark.parametrize("engine_payload", ["solar_park"], indirect=True,)
def test_simulation_measurements_contain_correct_asset_id_and_timestamps(engine_payload):
    simulated_power_measurement_list = simulate_asset_power_grid(
        engine_payload["config"],
        engine_payload["asset"],
    )
    ...
    # Every generated measurement must belong to the simulated asset.
    for measurement in simulated_power_measurement_list:
        assert measurement.asset_id == asset.asset_id
    ...
    # Measurement timestamps must match the generated time grid in order.
    for measurement, expected_time in zip(
        simulated_power_measurement_list,
        expected_time_grid,
    ):
        assert measurement.measurement_time == expected_time
...
def test_online_solar_asset_returns_rated_power_at_peak(engine_payload):
    ...
    assert active_power_kw == pytest.approx(40000.0)
...
def test_solar_factor_scales_active_power(engine_payload):
    ...
    assert active_power_kw == pytest.approx(30000.0)
...
def test_wind_differs_with_different_seed(engine_payload):
    ...
    config_with_new_seed = replace(engine_payload["config"], random_seed=2)
...
def test_wind_power_stays_within_expected_range(engine_payload):
    ...
    for seed in range(41):
        ...
        new_measurements = simulate_asset_power_grid(
...
def test_wind_factor_scales_active_power(engine_payload):
    """Isolate wind-factor scaling by using fresh generators with one seed."""
...
def test_hydro_factor_scales_active_power(engine_payload):
    hydro_factor = 0.5
    new_hydro_factor_context = replace(
        engine_payload["context"],
    ...
    active_power_kw = simulate_power_of_asset(
        engine_payload["asset"],
        new_hydro_factor_context,
        engine_payload["profile_data"],
    )

    assert active_power_kw == pytest.approx(
...
def test_biomass_factor_scales_active_power(engine_payload):
    biomass_factor = 0.5
    new_biomass_factor_context = replace(
        engine_payload["context"],
    ...
    active_power_kw = simulate_power_of_asset(
        engine_payload["asset"],
        new_biomass_factor_context,
        engine_payload["profile_data"],
    )

    assert active_power_kw == pytest.approx(engine_payload["asset"].rated_power_kw * biomass_factor)